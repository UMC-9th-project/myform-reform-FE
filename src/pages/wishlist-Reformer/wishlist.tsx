...
import WishlistCustomItemCard from '../../components/domain/wishlist/WishlisCustomItemCard';
import EmptyWishlist from '../../components/domain/wishlist/EmptyWishlist';
import type {
  WishlistItem,
  WishlistCustomItem,
} from '../../components/domain/wishlist/types';
...
const MOCK_CUSTOM_ITEMS: WishlistCustomItem[] = [
  {
    id: 1,
...
const ReformerWishlist = () => {
  ...
  const [customItems, setCustomItems] =
    useState<WishlistCustomItem[]>(MOCK_CUSTOM_ITEMS);
  ...
  // 마켓 아이템 제거 핸들러
  const handleRemoveFromMarket = (id: number) => {
    setMarketItems(marketItems.filter((item) => item.id !== id));
  };

  // 주문제작 아이템 제거 핸들러
  const handleRemoveFromCustom = (id: number) => {
...
export default ReformerWishlist;
