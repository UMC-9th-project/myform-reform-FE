...
import {
  createChatProposal,
  getChatProposalDetail,
} from '@/api/chat/chatProposalApi';
...
import { formatCurrencyInput, parseCurrency } from '@/utils/common/currency';
...
const ChatQuotationFormPage: React.FC = () => {
  ...
  const { chatRoomId, proposalId } = useParams<{
    chatRoomId?: string;
    proposalId?: string;
  }>();
  const mode: 'create' | 'edit' =
    location.state?.mode ?? (proposalId ? 'edit' : 'create');
  ...
  const [formData, setFormData] = useState<ChatQuotationFormData>({
    ...
    price: quotationData?.price
      ? formatCurrencyInput(quotationData.price.toString())
      : '',
    delivery: quotationData?.delivery
      ? formatCurrencyInput(quotationData.delivery.toString())
      : '',
    content: quotationData?.content ?? '',
    estimatedDays: quotationData?.estimatedDays?.toString() ?? '',
  });
  ...
  useEffect(() => {
    if (mode === 'edit' && !quotationData && proposalId) {
      getChatProposalDetail(proposalId)
        .then((data) => {
          setFormData({
            images: data.body.images.map((url: string) => ({
              file: null,
              preview: url,
            })),
            price: formatCurrencyInput(data.body.price.toString()), // 콤마 적용
            delivery: formatCurrencyInput(data.body.delivery.toString()), // 콤마 적용
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  {formData.images.length < MAX_IMAGES && (
                    ...
                    >
                      ...
                      >
                        <path
                          d="M9 25.5882V28.2941C9 29.0118 9.28973 29.7 9.80546 30.2075C10.3212 30.7149 11.0207 31 11.75 31H28.25C28.9793 31 29.6788 30.7149 30.1945 30.2075C30.7103 29.7 31 29.0118 31 28.2941V25.5882M13.125 14.7647L20 8M20 8L26.875 14.7647M20 8V24.2353"
                          stroke="#646F7C"
                          stroke-width="2"
            ...
            <div className="flex-1 space-y-4">
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    handleChange('price', formatCurrencyInput(e.target.value))
                  }
                  placeholder="견적 금액을 입력해주세요."
                  className="w-[55rem] p-4 body-b1-rg border border-[var(--color-gray-60)]"
                />
                <span className="absolute right-33 top-1/2 -translate-y-1/2 body-b1-rg">
                  원
                </span>
              </div>
              <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    handleChange('estimatedDays', onlyNumber);
                  }}
