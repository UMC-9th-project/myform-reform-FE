...
/*interface WriteFormProps {
  mode?: 'reform' | 'order';
  onSubmit: (data: ChatWriteFormData) => void;
  chatId?: number;
} */
...
const ChatWriteForm = () => {
  ...
  const { fromChatId, myRole } = location.state as { fromChatId: number; myRole: 'REFORMER' | 'USER' };
  ...
  console.log({
    ...
    if (myRole === 'REFORMER') navigate('/order/requests');
    else if (myRole === 'USER') navigate('/order/estimates');
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 bg-white text-gray-800">
      ...
      <h1 className="heading-h2-bd pb-6 border-b mb-8 border-[black]">
        {myRole === 'USER' ? '리폼 요청서 작성하기' : '리폼 견적서 작성하기'}
      </h1>
          ...
          <div className="w-3/4 body-b0-rg text-[var(--color-gray-50)] pr-25">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2">
                <span>-</span>
                <span>타인의 저작물 (예시 이미지 등)을 무단으로 캡쳐하여 업로드하는 것을 금지합니다.</span>
              </li>
              <li className="flex items-start gap-2">
                <span>-</span>
              ...
              <li className="flex items-start gap-2">
                <span>-</span>
                <span>구체적인 설명(원하는 스타일, 색상, 참고 이미지 등)을 작성할수록 더 정확한 제안을 받을 수 있습니다.</span>
              </li>
              <li className="flex items-start gap-2">
                <span>-</span>
                <span>부적절한 내용(비속어, 광고성 문구, 반복 게시물 등)은 사전 통보 없이 숨김 또는 삭제될 수 있습니다.</span>
              </li>
            </ul>
          </div>
        </div>
