    ...
    <div className={`flex w-[22rem] gap-2 ${isSent ? 'justify-end' : 'justify-start'}`}>        
        ...
        <div className={`${bgColor} ${borderRadiusClass} p-5 min-w-[22rem] shadow-sm`}>
          <h2 className="heading-h5-sb mb-2 text-black">요청서 전송완료!</h2>
            {type === 'sent' 
                ? <p className='body-b4-sb mb-3'>{nickname}님께<br />요청서가 성공적으로 전송되었습니다.</p>
                : <p className='body-b4-sb mb-3'>{nickname}님이<br />문의 내역에 따른 요청서를 보내왔습니다.</p>}
          ...
          <div className="bg-white rounded-xl px-3 py-4 space-y-3 shadow-sm text-sm mb-3">
            ...
            <div className="flex justify-between body-b4-sb">
              ...
              <span className="body-b3-sb">{minBudget.toLocaleString()}원 ~ {maxBudget.toLocaleString()}원</span>
            </div>
          </div>
