...
const RequireCard: React.FC<RequireCardProps> = ({
  minBudget,
  maxBudget,
  title,
  type,
  ...
  return (
    ...
    <div
      className={`flex w-[22rem] gap-2 ${isSent ? 'justify-end' : 'justify-start'}`}
    >
      {/* 4. 결제 카드 본체 */}
      <div
        className={`${bgColor} ${borderRadiusClass} p-5 min-w-[22rem] shadow-sm`}
      >
        ...
        <div className="bg-white rounded-xl px-3 py-4 space-y-3 shadow-sm text-sm mb-3">
          <div className="flex justify-between body-b4-sb border-b pb-2 border-[var(--color-line-gray-40)]">
            {/* title은 그냥 문자열 그대로 표시 */}
            <span className="overflow-hidden text-ellipsis whitespace-nowrap block">
              {title ?? '-'}
            </span>
          </div>
          <div className="flex justify-between body-b3-sb">
            <span className="body-b3-sb">희망 예산</span>
            <span className="body-b3-sb">
              {minBudget !== null && minBudget !== undefined
        ...
        <div className="flex body-b4-sb justify-center mb-3">
          추가 설명 및 견적 조율은 채팅으로 대화해보세요.
        </div>
        ...
        <button
          ...
          onClick={() =>
            navigate(`/chat/request/detail/${requestId}`, {
              state: {
        ...
        >
          {isSent ? '보낸 요청서 자세히 보기' : '받은 요청서 자세히 보기'}
        </button>
      </div>
    </div>
  );
